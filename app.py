...
/* ── MAIN BUTTON ── */
div[data-testid="stButton"]:not(.ex-btn) > button {
...
div[data-testid="stButton"]:not(.ex-btn) > button:hover {
    background: linear-gradient(135deg, #818cf8 0%, #6366f1 100%) !important;
    transform: translateY(-2px) !important;
    box-shadow: 0 8px 32px rgba(99,102,241,0.4) !important;
}

/* ── EXAMPLE BUTTONS — small and subtle ── */
.ex-btn-wrap button {
...
/* ── EXPANDER ── */
[data-testid="stExpander"] { background: #0f1120 !important; border: 1px solid rgba(255,255,255,0.07) !important; border-radius: 12px !important; margin: 0 !important; }
...
<style>
/* ── HERO ── */
.hero { padding: 64px 64px 52px; position: relative; overflow: hidden; border-bottom: 1px solid rgba(255,255,255,0.05); }
...
.hero-h1 { font-family: 'Syne', sans-serif; font-size: clamp(44px, 5.5vw, 72px); font-weight: 800; line-height: 1.0; color: #ffffff; margin-bottom: 20px; letter-spacing: -0.03em; }
...
.hero-body { font-size: 16px; font-weight: 300; color: rgba(255,255,255,0.45); max-width: 520px; line-height: 1.75; margin-bottom: 28px; }
...
/* ── SECTIONS — all have consistent 64px side padding ── */
.sec { padding: 40px 64px; border-bottom: 1px solid rgba(255,255,255,0.04); }
...
.sec-h2 { font-family: 'Syne', sans-serif; font-size: 21px; font-weight: 700; color: #ffffff; margin-bottom: 24px; letter-spacing: -0.02em; }
...
/* ── INTERP ── */
.interp { padding: 16px 20px; background: rgba(99,102,241,0.07); border: 1px solid rgba(99,102,241,0.18); border-radius: 12px; font-size: 14px; color: rgba(255,255,255,0.55); line-height: 1.7; }
...
/* ── FOOTER ── */
.footer { padding: 32px 64px; border-top: 1px solid rgba(255,255,255,0.05); display: flex; justify-content: space-between; align-items: center; gap: 24px; flex-wrap: wrap; }
...
/* ── ERROR ── */
.err { padding: 14px 18px; background: rgba(239,68,68,0.07); border: 1px solid rgba(239,68,68,0.2); border-radius: 10px; font-size: 13.5px; color: rgba(239,68,68,0.8); margin: 10px 0; }

/* ── MOBILE ── */
.mob-bar { display: none; padding: 10px 20px; background: rgba(99,102,241,0.08); text-align: center; font-size: 12px; color: rgba(255,255,255,0.35); border-bottom: 1px solid rgba(255,255,255,0.05); }
@media (max-width: 768px) {
    .mob-bar { display: block; }
    .ex-grid, .profile-row, .perf-row, .steps-row, .trust-row { grid-template-columns: 1fr 1fr !important; }
    .hero, .sec, .footer { padding-left: 20px !important; padding-right: 20px !important; }
}
</style>
""", unsafe_allow_html=True)
...
# ── DISCLAIMER ──
st.markdown("<div class='sec' style='border-bottom:none; padding-bottom:0'>", unsafe_allow_html=True)
st.markdown("""
<div class='disc'>
  <span>⚠</span>
...
# ── HOW IT WORKS ──
st.markdown("<div class='sec' style='padding-top:20px'>", unsafe_allow_html=True)
...
st.markdown("<div class='ex-btn-wrap'>", unsafe_allow_html=True)
ex_cols = st.columns(4, gap="small")
for i, (label, text) in enumerate(EXAMPLES):
    with ex_cols[i]:
        if st.button(f"↗ Use this example", key=f"ex_{i}"):
